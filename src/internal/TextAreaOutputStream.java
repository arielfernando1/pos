...
public class TextAreaOutputStream extends OutputStream
{
  ...
  {
    if (this.appender != null) this.appender.clear();
  }
  ...
  public synchronized void write(byte[] ba, int str, int len) {
    if (this.appender != null) this.appender.append(bytesToString(ba, str, len)); 
  }

  private static String bytesToString(byte[] ba, int str, int len) {
    try { return new String(ba, str, len, "UTF-8"); } catch (UnsupportedEncodingException thr) {  } return new String(ba, str, len);
  }
    ...
    implements Runnable
  {
    ...
    synchronized void append(String val) {
      this.values.add(val);
      if (this.queue) { this.queue = false; EventQueue.invokeLater(this); }
    }
    ...
    {
      if (this.clear) this.textArea.setText("");
      for (String val : this.values) {
        this.curLength += val.length();
        if ((val.endsWith("\n")) || (val.endsWith(EOL2))) {
          if (this.lengths.size() >= this.maxLines) this.textArea.replaceRange("", 0, ((Integer)this.lengths.removeFirst()).intValue());
